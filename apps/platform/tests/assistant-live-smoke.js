...
(async () => {
  ...
  const server = spawn(process.execPath, [SERVER], {
    env: Object.assign({}, process.env, { PORT: String(PORT), PLATFORM_DB_PATH: TEST_DB }),
  ...
  try {
    ...
    const schedule = await (await fetch(`${BASE}/api/platform/stylist-schedule-luminous-core?dayOffset=${row.day_offset}`)).json();
    ...
    const digest = await (await fetch(`${BASE}/api/assistant/digest`)).json();
